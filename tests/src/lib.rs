    ...
    ($name:ident::$rule:ident($input:expr) => $expected:expr) => {
        ...
        fn $name() {
            $name::Parser::with($input, |mut parser| {
                ...
                parser
                    .gss
                    .print(&mut File::create(concat!(env!("CARGO_MANIFEST_DIR"), "/../target/", stringify!($name), "-gss.dot")).unwrap())
                    .unwrap();
                parser
                    .sppf
                    .print(&mut File::create(concat!(env!("CARGO_MANIFEST_DIR"), "/../target/", stringify!($name), "-sppf.dot")).unwrap())
                    .unwrap();
            });
        }
    };
