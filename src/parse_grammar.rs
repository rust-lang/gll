    ...
    fn from_str(src: &str) -> Result<Self, Self::Err> {
        Grammar::parse_with(src, |_, g| {
            g.map_err(|err| match err {
                ParseError::TooShort(handle) => handle.source_line_column().end,
...
impl<'a, 'i, 's> Pattern<'a, 'i, 's> {
    fn lower(self) -> SPat {
        fn unescape<T>(handle: Handle<T>) -> String {
