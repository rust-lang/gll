...
struct RustField {
    ...
    /// Field paths into the tuples generated by `traverse!`.
    // FIXME(eddyb) find a better way to do this than field paths.
    paths: Vec<Vec<usize>>,
}
...
trait RuleWithFieldsMethods<Pat> {
    fn rust_fields(self, cx: &Context<Pat>) -> IndexMap<IStr, RustField>;
...
impl<Pat: RustInputPat> RuleWithFieldsMethods<Pat> for RuleWithFields {
    fn rust_fields(self, cx: &Context<Pat>) -> IndexMap<IStr, RustField> {
        ...
        let child_fields = |rule, i| {
            ...
            let mut fields = child.rust_fields(cx);
                    ...
                    for (name, field) in child_fields(i) {
                        ...
                        match fields.entry(name) {
                            Entry::Occupied(entry) => {
                                ...
                                entry.paths.extend(field.paths);
                            }
                            Entry::Vacant(entry) => {
                                entry.insert(field);
                            }
    ...
    Pat: RustInputPat,
{
    ...
    rule_ty_def
        ...
        + impl_rule_from_forest(name, rule.rule, &rust_adt, cx, rules)
        + impl_rule_one_and_all(name, rule.rule, &rust_adt, cx, rules)
}
...
{
    let ident = Src::ident(&cx[name]);
    let field_handle_expr = |field: &RustField| {
        let paths_expr = field.paths.iter().map(|path| {
            // HACK(eddyb) workaround `quote!(#i)` producing `0usize`.
            let path = path
                .iter()
                .cloned()
    ...
    let (one, all) = match rust_adt {
        RustAdt::Enum(variants) => {
            ...
            let variants_kind = variants
                .values()
                .map(|(v_rule, _)| v_rule.node_kind(cx, rules))
                .collect::<Vec<_>>();
                    ...
                    forest.all_choices(node).flat_map(move |node| {
                        match node.kind {
                            #(#variants_kind_src => Iter::#i_ident(
                                traverse!(all(forest, node, r: traverse!(typeof(_) #variants_shape) => *r) #variants_shape)
                                    .into_iter()
                                    .map(move |r| {
                                        #ident::#variants_from_forest_ident(self.forest, node, r)
                                    })
        ...
        RustAdt::Struct(_) => {
            ...
            (
                ...
                quote!(
                    traverse!(all(forest, node, r: traverse!(typeof(_) #shape) => *r) #shape)
                        .into_iter()
                        .map(move |r| {
                            #ident::from_forest(self.forest, node, r)
                        })
